...
function createSchema(db) {
  ...
  // Schema auto-migration: ensure max_uses, user_id, user_email columns exist in cdkeys
  try {
    ...
    if (cdkeyCols && !cdkeyCols.some(c => c.name === 'user_id')) {
      db.exec('ALTER TABLE cdkeys ADD COLUMN user_id TEXT');
    }
    if (cdkeyCols && !cdkeyCols.some(c => c.name === 'user_email')) {
      db.exec('ALTER TABLE cdkeys ADD COLUMN user_email TEXT');
    }
    db.exec(`
      CREATE INDEX IF NOT EXISTS idx_cdkeys_user_id ON cdkeys(user_id);
      CREATE INDEX IF NOT EXISTS idx_cdkeys_user_email ON cdkeys(user_email);
    `);
  } catch (e) {}

  // Schema auto-migration: ensure last_visited_at, status, deleted_at, delete_reason exist in sites
  try {
    const siteCols = db.prepare('PRAGMA table_info(sites)').all();
    if (siteCols && !siteCols.some(c => c.name === 'last_visited_at')) {
      db.exec('ALTER TABLE sites ADD COLUMN last_visited_at TEXT');
    }
    if (siteCols && !siteCols.some(c => c.name === 'status')) {
      db.exec("ALTER TABLE sites ADD COLUMN status TEXT DEFAULT 'active'");
    }
    if (siteCols && !siteCols.some(c => c.name === 'deleted_at')) {
      db.exec('ALTER TABLE sites ADD COLUMN deleted_at TEXT');
    }
    if (siteCols && !siteCols.some(c => c.name === 'delete_reason')) {
      db.exec('ALTER TABLE sites ADD COLUMN delete_reason TEXT');
    }
    db.exec(`
      CREATE INDEX IF NOT EXISTS idx_sites_status ON sites(status);
      CREATE INDEX IF NOT EXISTS idx_sites_last_visited_at ON sites(last_visited_at);
    `);
  } catch (e) {}
}
...
function rowToSite(row) {
  if (!row) return null;
  const siteId = row.site_id || row.siteId || row.id;
  const site = {
    siteId,
    subdomain: row.subdomain || row.custom_path || row.customPath || siteId,
    url: row.url,
    cdkey: row.cdkey,
    type: row.type || 'html',
    fileCount: (row.file_count !== undefined && row.file_count !== null) 
      ? Number(row.file_count) 
      : ((row.fileCount !== undefined && row.fileCount !== null) ? Number(row.fileCount) : 1),
    storage: row.storage || 'local',
    duration: row.duration || '3d',
    expiresAt: row.expires_at || row.expiresAt || null,
    userId: row.user_id || row.userId || null,
    userEmail: row.user_email || row.userEmail || null,
    visits: (row.visits !== undefined && row.visits !== null) ? Number(row.visits) : 0,
    lastVisitedAt: row.last_visited_at || row.lastVisitedAt || null,
    status: row.status || 'active',
    deletedAt: row.deleted_at || row.deletedAt || null,
    deleteReason: row.delete_reason || row.deleteReason || null,
    renewedAt: row.renewed_at || row.renewedAt || null,
    lastCdkey: row.last_cdkey || row.lastCdkey || null,
    createdAt: row.created_at || row.createdAt || new Date().toISOString()
  };
  const customPath = row.custom_path || row.customPath;
  if (customPath) site.customPath = customPath;
  if (row.extra_meta) {
    try {
...
function siteToRow(site) {
  ...
  const lastVisitedAt = site.lastVisitedAt || site.last_visited_at || null;
  const status = site.status || 'active';
  const deletedAt = site.deletedAt || site.deleted_at || null;
  const deleteReason = site.deleteReason || site.delete_reason || null;
...
function cdkeyToRow(keyObj) {
  return {
    ...
    user_id: keyObj.userId || null,
    user_email: keyObj.userEmail || null
  };
}
...
const db = {
  ...
  sites: {
    ...
    findByUserIdOrEmail(userId, userEmail, includeDeleted = false) {
      if (!userId && !userEmail) return [];
      const cleanEmail = userEmail ? userEmail.trim().toLowerCase() : null;
      const dbInst = initDatabase();
      if (dbInst) {
        let rows = [];
        const statusClause = includeDeleted ? '' : " AND (status IS NULL OR status != 'auto_deleted')";
        if (userId && cleanEmail) {
          ...
          rows = dbInst.prepare(`
            SELECT * FROM sites 
            WHERE user_email IS NOT NULL AND LOWER(user_email) = ?
            ${statusClause}
            ORDER BY created_at DESC
          `).all(cleanEmail);
        }
      ...
      return this.getAll().filter(s => {
        if (!includeDeleted && s.status === 'auto_deleted') return false;
    ...
    getSitesForAutoClean() {
      const dbInst = initDatabase();
      if (dbInst) {
        const rows = dbInst.prepare("SELECT * FROM sites WHERE status IS NULL OR status != 'auto_deleted'").all();
        return rows.map(rowToSite);
      }
      return this.getAll().filter(s => s.status !== 'auto_deleted');
    },
      ...
      if (dbInst) {
        const stmt = dbInst.prepare(`
          INSERT INTO sites (
            ...
            last_visited_at, status, deleted_at, delete_reason,
        ...
        stmt.run(
          r.site_id, r.subdomain, r.custom_path, r.url, r.cdkey,
          r.type, r.file_count, r.storage, r.duration, r.expires_at,
          r.user_id, r.user_email, r.visits,
          r.last_visited_at, r.status, r.deleted_at, r.delete_reason,
          r.renewed_at, r.last_cdkey,
          r.created_at, r.extra_meta
        );
    ...
    recordSuccessVisit(siteId) {
      if (!siteId) return;
      const now = new Date().toISOString();
      const dbInst = initDatabase();
      if (dbInst) {
        try {
          dbInst.prepare('UPDATE sites SET visits = visits + 1, last_visited_at = ? WHERE site_id = ?').run(now, siteId);
        } catch (e) {
          console.warn('recordSuccessVisit error:', e.message);
        }
        return;
      }
      const list = this.getAll();
    ...
    markAutoDeleted(siteId, reason = '周期内无有效访问自动清理') {
      if (!siteId) return false;
      const now = new Date().toISOString();
      const dbInst = initDatabase();
      if (dbInst) {
        try {
          const res = dbInst.prepare(`
            UPDATE sites 
            SET status = 'auto_deleted', deleted_at = ?, delete_reason = ? 
            WHERE site_id = ?
          `).run(now, reason, siteId);
          ...
          console.error('markAutoDeleted error:', e.message);
          return false;
        }
      }
      const list = this.getAll();
    ...
    incrementVisits(siteId) {
      this.recordSuccessVisit(siteId);
    },
    ...
    restoreSite(siteId) {
      ...
      if (dbInst) {
        try {
          ...
          console.error('restoreSite error:', e.message);
          return false;
        }
      }
      const list = this.getAll();
    ...
    findByUserIdOrEmail(userId, userEmail) {
      ...
      if (dbInst) {
        ...
        if (userId && cleanEmail) {
          ...
          rows = dbInst.prepare('SELECT * FROM cdkeys WHERE user_id = ? ORDER BY created_at DESC').all(userId);
        } else {
          rows = dbInst.prepare('SELECT * FROM cdkeys WHERE user_email IS NOT NULL AND LOWER(user_email) = ? ORDER BY created_at DESC').all(cleanEmail);
        }
        return rows.map(rowToCdkey);
      }
      return this.getAll().filter(k => {
        if (userId && k.userId === userId) return true;
        if (cleanEmail && k.userEmail && k.userEmail.toLowerCase() === cleanEmail) return true;
        return false;
      });
    },
